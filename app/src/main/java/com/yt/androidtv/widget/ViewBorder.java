...
import com.yt.androidtv.utils.Utils;

/**
 * Created by zyk on 2016/3/15.
 *  画View的边框
 *  需要画边框的view 需要设置 android:focusableInTouchMode="true"，
 *  若 view有其他事件监听(setOnClickListener) android:clickable="true"不是必须的
 *  注意 ViewBorder的层次关系，若ViewBorder不在顶层可能被其他控件遮到，而不能显示。
 *  建议 ViewBorder放在所有View的根节点的后面。如：
 ...
 *  </RelativeLayout>
 */
public class ViewBorder extends View {
    private static final String TAG = "ViewBorder";
    ...
    public ViewBorder(Context context, View view) {
        super(context, null, 0);
        // 如果是单独添加，就将view加进来.
        if (view != null) {
...
//        setVisibility(View.GONE);
    ...
    protected void onDraw(Canvas canvas) {
        noDrawViewBorder(canvas);
    }

    private void noDrawViewBorder(Canvas canvas) {
        canvas.save();
        // 绘制阴影.
        onDrawShadow(canvas);
    ...
    public void setFocusView(View view, float scale) {
        if (view != null && mFocusView != view) {
            ...
            mFocusView = view;
            mNewFocus = view;
            mFocusView.animate().scaleX(scale).scaleY(scale).setDuration(TRAN_DUR_ANIM).setListener(new Animator.AnimatorListener() {
                @Override
    ...
    public void runTranslateAnimation(View toView, float scaleX, float scaleY) {
         ...
         */
        if (isTvScreen) {
            x = Utils.dip2px(this.getContext(), x);
            y = Utils.dip2px(this.getContext(), y);
        }
